...
// Once an event is done, it can no longer be edited — only deleted.
$adminBackSuffix = ($currentUser['role'] === 'admin') ? ('?organizer_id=' . $event->getOrganizerId()) : '';
if ($event->getStatus() === 'done') {
    app_set_flash('error', 'This event has already ended and can no longer be edited. You can still delete it.');
    app_redirect('/dashboard.php' . $adminBackSuffix);
}
            ...
            <div class="card-body">
                ...
                <form method="post" action="<?php echo app_url('/edit-event.php?id=' . $eventId); ?>">
                    ...
                    <div class="d-flex gap-2 mt-4">
                        ...
                        <a href="<?php echo app_url('/dashboard.php' . $adminBackSuffix); ?>" class="btn btn-outline-secondary">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
