...
// If an admin arrived here from "Manage as Organizer", create the event under
// that organizer's account instead of the admin's own account.
$actingOrganizer = null;
$organizerIdParam = (int) ($_GET['organizer_id'] ?? $_POST['organizer_id'] ?? 0);
if ($currentUser['role'] === 'admin' && $organizerIdParam > 0) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name === '' || $location === '' || $eventDate === '' || $startTime === '' || $endTime === '' || $description === '') {
        ...
        $event = new Event($db);
        $event->setOrganizerId($actingOrganizer !== null ? (int) $actingOrganizer->getId() : (int) $currentUser['id']);
        $event->setName($name);
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                ...
                <?php if ($actingOrganizer !== null): ?>
                    <div class="alert alert-info small">Creating this event on behalf of organizer <strong><?php echo htmlspecialchars($actingOrganizer->getName(), ENT_QUOTES, 'UTF-8'); ?></strong>.</div>
                <?php endif; ?>
                ...
                <form method="post" action="">
                    <?php if ($actingOrganizer !== null): ?>
                        <input type="hidden" name="organizer_id" value="<?php echo $actingOrganizer->getId(); ?>">
                    <?php endif; ?>
                    <div class="row g-3">
                        <div class="col-md-6">
